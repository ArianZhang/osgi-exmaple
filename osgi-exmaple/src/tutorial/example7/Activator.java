...
/**
 * This class implements a bundle that uses a spell checker service to check the
 * spelling of a passage. This bundle is essentially identical to Example 5, in
 * that it uses the Service Tracker to monitor the dynamic availability of the
 * spell checker service. When starting this bundle, the thread calling the
 * start() method is used to read passages from standard input. You can stop
 * spell checking passages by entering an empty line, but to start spell
...
public class Activator implements BundleActivator {
	...
	/**
	 * Implements BundleActivator.start(). Creates a Service Tracker object to
	 * monitor spell checker services. Enters a spell check loop where it reads
	 * passages from standard input and checks their spelling using the spell
	 * checker service. (NOTE: It is very bad practice to use the calling thread
